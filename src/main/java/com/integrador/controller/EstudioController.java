...
public class EstudioController extends GenericoController<Estudio, EstudioDAO> {
	...
	public ResponseEntity<Estudio> buscarPorEmailESenha(@PathVariable String email, @PathVariable String senha) {
		Estudio p = super.t.buscarPorEmailESenha(email, senha);
		if (p != null)
	...
	public ResponseEntity<List<Estudio>> buscarPorNome(@PathVariable String nome) {
		List<Estudio> lista = t.buscarPorNome(nome);
		if (lista != null)
	...
	public ResponseEntity<List<Estudio>> buscarPorProximidade(@PathVariable Double latitude,
			@PathVariable Double longitude, @PathVariable Double raio) {
	...
	public ResponseEntity<List<Estudio>> buscarPorFiltro(@PathVariable Double latitude, @PathVariable Double longitude,
			@PathVariable Double raio, @PathVariable Double preco, @PathVariable Time inicio, @PathVariable Time fim,
			@PathVariable int ordenacao, @RequestBody Localizacao localizacao) {
		List<Estudio> lista = t.buscarPorProximidadeLocalizacaoPrecoHorarioDisponivel(latitude, longitude, raio,
				localizacao, preco, inicio, fim, ordenacao);
		if(lista!=null)
		return ResponseEntity.ok(lista);
		return ResponseEntity.notFound().build();
	}
